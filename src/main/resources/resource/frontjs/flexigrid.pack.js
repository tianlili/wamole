		...
		var g = {
			...
			addData : function(data) {
				...
				$("[id$='_view']").click(function(){
					...
					window.frames.location.href = "../../project/"+ name +"/view";
				});
			},
				...
				$.ajax({
					...
					success : function(data) {
						...
						for(var i=0; i<data.length; i++){
							...
							pdata.rows[i].id = "id";
							pdata.rows[i].cell = data[i];
							pdata.rows[i].cell.path = decodeURIComponent(pdata.rows[i].cell.path);
							pdata.rows[i].cell.view = "<div style='width:20px' class='fview'><img id=" + pdata.rows[i].cell.name + "_view src='../resource/frontcss/images/view.ico' width=20px height=20px/></div>";
						}
						g.addData(pdata);
					},
